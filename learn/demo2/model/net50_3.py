...
from learn.demo2.model.selayer import SELayer
...
class SiameseNetwork(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=1, padding=4, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        ...
        # 增加SE模块
        expansion = 1
        self.se_layer1 = SELayer(64 * expansion)
        self.se_layer2 = SELayer(128 * expansion)
        self.se_layer3 = SELayer(256 * expansion)
        self.se_layer4 = SELayer(512 * expansion)
    ...
    def forward_once(self, x):
        ...
        out = self.se_layer1(out)  # 添加SE模块
        out = self.layer2(out)  #-->（batch,128,16,16）
        out = self.se_layer2(out)  # 添加SE模块
        out = self.layer3(out)  #-->（batch,256,8,8）
        out = self.se_layer3(out)  # 添加SE模块
        out = self.layer4(out)  #-->（batch,512,4,4）
        out = self.se_layer4(out)  # 添加SE模块
